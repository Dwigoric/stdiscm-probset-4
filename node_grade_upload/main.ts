...
app.post("/post-grade", async (req, res) => {
    ...
    if (role !== "faculty") return res.status(403).json({ message: "Unauthorized" });
    if (!grade || !courseCode || !studentId) return res.status(400).json({ message: "Missing at least one of required fields: studentId, grade, course" });
    ...
    if (typeof courseCode !== "string") {
        return res.status(400).json({ message: "Course code must be a string" });
    }
    ...
    if (!instructor.courses.includes(course._id)) return res.status(403).json({ message: "Instructor not assigned to this course" });
    
    const student = await User.findOne({ userId: studentId });
    if (!student) return res.status(404).json({ message: "Student not found" });
    if (!student.courses.includes(course._id)) return res.status(400).json({ message: "Student not enrolled in this course" });
    ...
    if (!insertedGrade) return res.status(500).json({ message: "Failed to insert grade" });
    return res.status(201).json({ message: "Grade posted successfully" });
});
